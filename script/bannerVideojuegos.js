...
async function loadAllPosts() {
    try {
        ...
        const videogameCategories = ['Xbox', 'Nintendo', 'PlayStation', 'PC', 'Indie'];
        allPosts = allPostsFromAPI.filter(post => 
            videogameCategories.includes(post.category)
        );
...
function showDefaultBanner() {
    const bannerSection = document.getElementById('bannerVideojuegos');
    ...
    if (bannerSection && bannerLink) {
        ...
        setTimeout(() => {
            bannerLink.href = '#';
            bannerSection.className = "bg-[url(./img/2x1_NSwitch_DonkeyKongCountryReturnsHD_image1600w.jpg)] bg-cover bg-center w-full h-150 md:h-200 content-end border-b-3 border-(--NeonGrey) lg:h-[90vh]";
            
            bannerSection.innerHTML = `
                <div class="w-full fondoBanner lg:p-6 px-4 lg:px-20">
                    <h1 class="lg:text-6xl">Últimas noticias de videojuegos</h1>   
                    <div class="info flex items-center gap-1 pb-2 lg:h-[10vh]">
                        ...
                        <div class="date lg:flex lg:gap-3">
                            ...
                            <p class="hidden lg:block lg:text-3xl lg:font-light">Videojuegos</p>
                        </div>
                    </div>  
                </div>
            `;
